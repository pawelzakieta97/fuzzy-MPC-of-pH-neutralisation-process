classdef FuzzyController < handle
    ...
    methods
        function obj = FuzzyController(controllers, op_models, membership_fun)
            ...
            obj.op_models = op_models;
            obj.membership_fun = membership_fun;
            obj.weights = zeros(2000,length(controllers));
        end
        function u = get_steering(obj, current_model)
            total_weight = 0;
            steering = 0;
            for i=1:length(obj.controllers)
                weight = obj.membership_fun(obj.op_models(i), current_model);
